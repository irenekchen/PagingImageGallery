...
class ImageGalleryViewController : UIViewController {
    ...
    func updateCaptionViewConstraints() {
    ...
    func numberOfImages() -> Int {
        return delegate.dziFiles.count ?? 0
    }
    
    
    func reloadData() {
        if delegate.dziFiles.count > 0 && pageIndex < delegate.dziFiles.count {
            pageViewController.setViewControllers([makeImageViewController(imageIndex:pageIndex, dziFile:delegate.dziFiles[pageIndex])],
                                                  direction: .forward,
                                                  animated: true,
                                                  completion: nil)
    ...
    func makeImageViewController(imageIndex:Int, dziFile: Dzifile) -> ImageViewController {
...
extension ImageGalleryViewController : UIPageViewControllerDataSource {
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        if let imageVC = viewController as? ImageViewController {
            if let index = imageVC.imageIndex {
                if index > 0 {
                    let dziFile = delegate.dziFiles[index-1]
                    print("Image view \(index-1) is being made")
                    return makeImageViewController(imageIndex:index-1, dziFile: dziFile)
                }
            }
        }
        return nil
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        if let imageVC = viewController as? ImageViewController {
            if let index = imageVC.imageIndex {
                if index < delegate.dziFiles.count-1 {
                    let dziFile = delegate.dziFiles[index+1]
                    print("Image view \(index+1) is being made")
                    return makeImageViewController(imageIndex:index+1, dziFile: dziFile)
                }
